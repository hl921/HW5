...
print ([x.years_UM for x in programmers])
...
prod_iter = iter(map(lambda x: x.num_programs/x.years_UM, programmers))
prod_list = [x for x in prod_iter]
...
long_names = filter((lambda a: len(a) > 4), [x.name for x in programmers])
print (type(long_names))
long_names_list = [Student(x) for x in long_names]
...
for ln in main_filterer(provided_file_names):
    print (ln.rstrip('\n'), end=" ")
#####
# print(list(len_check(readfiles(provided_file_names))))
